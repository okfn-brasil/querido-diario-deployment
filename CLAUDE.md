...
```bash
make spider-setup                                     # sincroniza o ambiente via uv (uma vez)
...
O código dos raspadores está em `../querido-diario/querido_diario_raspadores/`. `make run-spider` conecta automaticamente ao Postgres e ao Garage (S3) do cluster kind local (port-forward + credenciais do secret `app-secret`) — não precisa de `.local.env` pra esse caso. Crie `../querido-diario/querido_diario_raspadores/.local.env` só se quiser apontar pra outro ambiente (ex: Revoada); o que estiver lá tem prioridade sobre a auto-conexão.
